...
BookReader.prototype.init = function() {
  ...
  if (this.options.showToolbar) {
    ...
    this.refs.$BRtoolbar = this.refs.$BRtoolbar || this.$('.BRtoolbar');
  }
  if (this.options.showNavbar) { // default navigation
    const $navBar = this.initNavbar();
    this.refs.$BRnav = $navBar;
...
BookReader.prototype.enterFullscreen = async function(bindKeyboardControls = true) {
  ...
  const spacingIn = this.activeMode instanceof Mode1Up ? this.activeMode.mode1UpLit.SPACING_IN : null;
  this._fullscreenState = {
    reduce: this.reduce,
    spacingIn,
    toolbarVisible: this.refs.$BRtoolbar?.is(':visible') ?? false,
    navVisible: this.refs.$BRnav?.is(':visible') ?? false,
  };
  ...
  this.refs.$BRtoolbar?.stop(true, true).fadeOut(180);
  this.refs.$BRnav?.stop(true, true).fadeOut(180);
  this.refs.$brContainer.addClass('BRfullscreen-container');
  // prioritize class updates so CSS can propagate
  this.updateBrClasses();
  if (this.activeMode instanceof Mode1Up) {
    this.activeMode.mode1UpLit.scale = 1;
  ...
  if (typeof window !== 'undefined' && window.llgBridge && typeof window.llgBridge.bookReaderEnterFullscreen === 'function') {
    try {
      window.llgBridge.bookReaderEnterFullscreen();
    } catch (err) {
      console.error('bookReaderEnterFullscreen bridge call failed', err);
    }
  }
  ...
  if (this.activeMode instanceof Mode1Up) {
    await this._applyMode1UpFullscreenScale(currentIndex);
  }
...
BookReader.prototype.exitFullScreen = async function () {
  ...
  this.trigger(BookReader.eventNames.fullscreenToggled);
  if (typeof window !== 'undefined' && window.llgBridge && typeof window.llgBridge.bookReaderExitFullscreen === 'function') {
    try {
      window.llgBridge.bookReaderExitFullscreen();
    } catch (err) {
      console.error('bookReaderExitFullscreen bridge call failed', err);
    }
  }
  ...
  const spacingToRestore = (this.activeMode instanceof Mode1Up && typeof this._fullscreenState?.spacingIn === 'number')
    ? this._fullscreenState.spacingIn
    : null;
  if (spacingToRestore !== null) {
    const mode1UpLit = this.activeMode.mode1UpLit;
    mode1UpLit.SPACING_IN = spacingToRestore;
    mode1UpLit.updatePages();
  }

  this.resize();
  this.refs.$brContainer.removeClass('BRfullscreen-container');
  if (this._fullscreenState) {
    if (typeof this._fullscreenState.reduce === 'number') {
      this.reduce = this._fullscreenState.reduce;
    }
    if (this._fullscreenState.toolbarVisible) {
      this.refs.$BRtoolbar?.stop(true, true).fadeIn(180);
    }
    if (this._fullscreenState.navVisible) {
      this.refs.$BRnav?.stop(true, true).fadeIn(180);
    }
...
BookReader.prototype._applyMode1UpFullscreenScale = async function(pageIndex) {
  const mode1Up = this._modes?.mode1Up;
  const mode1UpLit = mode1Up?.mode1UpLit;
  const $container = this.refs.$brContainer;
  if (!mode1UpLit || !$container?.length) {
    return;
  }
  ...
  const horizontalWorld = page.widthInches + (mode1UpLit.SPACING_IN * 2);
  const verticalWorld = page.heightInches + (mode1UpLit.SPACING_IN * 2);
  const widthPxAtScale1 = mode1UpLit.coordSpace.worldUnitsToRenderedPixels(horizontalWorld);
  const heightPxAtScale1 = mode1UpLit.coordSpace.worldUnitsToRenderedPixels(verticalWorld);
  if (!(widthPxAtScale1 && heightPxAtScale1)) {
    return;
  }

  const scale = Math.min(containerWidth / widthPxAtScale1, containerHeight / heightPxAtScale1);
  if (!isFinite(scale) || scale <= 0) {
    return;
  }

  this.reduce = scale;
  mode1UpLit.scale = scale;
  mode1UpLit.requestUpdate();
  await mode1UpLit.updateComplete;
};
